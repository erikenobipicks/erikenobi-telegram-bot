...
def menu_markup() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("Contacto", url=CONTACTO_TELEGRAM)],
        ...
        [
            ...
            InlineKeyboardButton("OVER 2.5 FT", callback_data="prepartido"),
        ],
    ])
...
def menu_texto() -> str:
    return (
        ...
        "Alertas de futbol centradas en *GOLES* y *CORNERS*, "
        "mas un servicio manual aparte para *Over 2.5 FT prepartido*.\n\n"
...
def info_texto() -> str:
    return (
        "*Como funciona*\n\n"
        "Este servicio ofrece alertas basadas en analisis estadistico "
        "y seguimiento de partidos en tiempo real.\n\n"
        "*GOLES*\n"
        "Alertas de gol en directo y contexto del partido en vivo.\n\n"
        "*CORNERS*\n"
        "Alertas especializadas en mercados de corners en vivo.\n\n"
        "*COMBO*\n"
        "Acceso completo a GOLES + CORNERS.\n\n"
        "*OVER 2.5 FT PREPARTIDO*\n"
        "Servicio manual aparte, con analisis previo y seguimiento propio.\n\n"
        "*Metodos de pago disponibles*\n"
        "Stripe | PayPal | Bizum | Revolut\n\n"
        "*Aviso de responsabilidad*\n"
        "Este servicio es unicamente informativo. "
        "Cada usuario es responsable de sus propias decisiones."
    )


def stats_fallback_texto() -> str:
    return (
        "*Rendimiento del servicio*\n\n"
        "*GOLES*\n"
        "Acierto estimado actual: *+70%*\n"
        "Alertas de gol en directo con seguimiento del partido.\n\n"
        "*CORNERS*\n"
        "Acierto estimado actual: *+80%*\n"
        "Alertas en vivo basadas en estadisticas y momentum.\n\n"
        "*COMBO*\n"
        "Rendimiento estimado combinado: *+75%*\n"
        "Acceso completo a GOLES + CORNERS.\n\n"
        "*OVER 2.5 FT PREPARTIDO*\n"
        "Servicio manual independiente con resultados verificados.\n\n"
        "_Los datos en tiempo real no estan disponibles en este momento. Intentalo mas tarde._"
    )


def registrar_pago_pendiente(user, plan: str) -> None:
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
...
def texto_plan_goles(stats: dict | None) -> str:
    strike_real = _get_strike_tipo(stats, "gol")
    strike_txt = f"*{strike_real}* (ultimo mes)" if strike_real else "*+70% estimado*"
    return (
        f"*PLAN GOLES*\n\n"
        ...
        "- Informacion estadistica del partido\n"
        "- Seguimiento del contexto en vivo\n\n"
        "Selecciona tu metodo de pago preferido:"
    )


def texto_plan_corners(stats: dict | None) -> str:
    strike_real = _get_strike_tipo(stats, "corner")
    strike_txt = f"*{strike_real}* (ultimo mes)" if strike_real else "*+80% estimado*"
    return (
        f"*PLAN CORNERS*\n\n"
        f"Strike real: {strike_txt}\n"
        f"Precio: *{PRECIO_CORNERS}*\n\n"
        "Incluye:\n"
        "- Alertas especializadas en corners\n"
        "- Datos de momentum y presion ofensiva\n"
        "- Estadisticas del partido en vivo\n\n"
        "Selecciona tu metodo de pago preferido:"
    )


def texto_plan_combo(stats: dict | None) -> str:
    strike_goles = _get_strike_tipo(stats, "gol")
    strike_corner = _get_strike_tipo(stats, "corner")

    if strike_goles and strike_corner:
        strike_txt = f"Goles {strike_goles} | Corners {strike_corner} (ultimo mes)"
    elif strike_goles or strike_corner:
        strike_txt = f"*{strike_goles or strike_corner}* (ultimo mes)"
    else:
    ...
    return (
        ...
        "- GOLES\n"
        "- CORNERS\n\n"
        "Perfecto para seguir ambos tipos de alertas y elegir segun el volumen del dia.\n\n"
        "Selecciona tu metodo de pago preferido:"
    )


def prepartido_markup() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("Consultar acceso", url=CONTACTO_TELEGRAM)],
        [InlineKeyboardButton("Volver al menu", callback_data="menu")],
    ])


def texto_plan_prepartido() -> str:
    return (
        "*OVER 2.5 GOLES FT PREPARTIDO*\n\n"
        "Sistema manual aparte del servicio de goles y corners.\n"
        "Se trabaja con analisis previo, filtro propio y publicacion especifica.\n\n"
        "Que vas a encontrar:\n"
        "- Seleccion manual antes del partido\n"
        "- Seguimiento propio del sistema\n"
        "- Servicio separado de GOLES, CORNERS y COMBO\n\n"
        "Si quieres acceso o mas informacion, escribeme y te explico el alta."
    )
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user:
        ...
        if acceso:
            await message.reply_text(
                ...
                "Pulsa el boton para obtener tu enlace de entrada.",
    ...
    if payload in ("goles", "corners", "combo") and user:
        registrar_pago_pendiente(user, payload)

    if payload == "goles":
        await message.reply_text(
            texto_plan_goles(get_stats_reales()),
    ...
    if payload == "corners":
        await message.reply_text(
            texto_plan_corners(get_stats_reales()),
    ...
    if payload == "combo":
        await message.reply_text(
            texto_plan_combo(get_stats_reales()),
            reply_markup=pago_markup("combo"),
            parse_mode="Markdown",
        )
        return

    if payload == "prepartido":
        await message.reply_text(
            texto_plan_prepartido(),
            reply_markup=prepartido_markup(),
            parse_mode="Markdown",
        )
        return
    ...
    if payload == "free":
        await message.reply_text(
            "*Canal FREE*\n\nDesde aqui puedes entrar al canal gratuito.",
    ...
    if payload == "stats":
        ...
        texto = _formatear_stats_reales(stats) if stats and (stats.get("globales") or stats.get("ultimo_mes")) else stats_fallback_texto()
        await message.reply_text(
            texto,
            reply_markup=volver_markup(),
            parse_mode="Markdown",
...
async def seleccionar_plan(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if plan in ("goles", "corners", "combo"):
        registrar_pago_pendiente(user, plan)
    ...
    if plan == "free":
        await query.edit_message_text(
            "*Canal FREE*\n\nDesde aqui puedes acceder al canal gratuito.",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("Entrar al canal FREE", url=LINK_FREE)],
                [InlineKeyboardButton("Volver al menu", callback_data="menu")],
            ]),
    ...
    if plan.startswith("bizum:"):
        ...
        await query.edit_message_text(
            ...
            f"Numero Bizum: *{BIZUM}*\n\n"
            "Realiza el pago y envia el comprobante en este chat.\n"
            "Una vez validado recibiras el acceso automaticamente.",
            reply_markup=volver_markup(),
            parse_mode="Markdown",
        )
        return
